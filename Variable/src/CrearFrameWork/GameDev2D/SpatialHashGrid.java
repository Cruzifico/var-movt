package CrearFrameWork.GameDev2D;
...
public class SpatialHashGrid {
	...
	public SpatialHashGrid(float worldWidth, float worldHeight, float cellSize){
		...
		for(int i = 0; i < numCells; i++){
			dynamicCells[i] = new ArrayList<GameObject>();
		}
	...
	public void clearDynamicCells(int obj){
	...
	public List<GameObject> getPotentialColliders(GameObject obj){
		...
		while(i <= 3 && (cellId = cellIds[i++]) != -1){
			...
			for(int j = 0; j < len; j++){
				...
				if(!foundObjects.contains(collider)) // might be slow
